...
_ASMoutput = namedtuple('ASMoutput', ['output', 'output_full','loss'])
    ...
    def forward(self, input, target):
        ...
        for i in range(len(cutoff_values) - 1):
            ...
            if i == 0:
                ...
                cluster_logprob = self.log_softmax(cluster_output)
        ...
        head_logprob = self.log_softmax(head_output)
        output += head_logprob.gather(1, gather_inds.unsqueeze(1)).squeeze()
        loss = (-output).mean()
        
        if self.get_full_prob:
            with torch.no_grad():
                full_output=self.log_prob(input)
        else:
            full_output=out

        return _ASMoutput(output, full_output, loss)
